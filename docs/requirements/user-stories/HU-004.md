...
**Quiero** ingestar, crear, modificar y dar seguimiento a las órdenes de compra a proveedores,  
...
### CA-001: Recibir/Crear orden de compra

- **Dado** que necesito reponer productos de un proveedor
- **Cuando** Ingreso via csv, API o manualmente creo una OC seleccionando proveedor, productos, cantidades y fecha de emisión
...
- **Entonces** el sistema cambia el estado a "Recibida", registra la fecha real de llegada, utiliza esa fecha para calcular el Lead Time real y promediar con el Lead Time histórico advirtiendo al usuario si existe una variación mayor al 10%, actualiza el inventario en tránsito, y genera el ingreso automático a inventario
...
- **Entonces** el sistema mantiene el estado "Parcialmente Recibida" y permite registrar el resto posteriormente, actualiza el inventario en tránsito, y genera el ingreso automático a inventario, utiliza esta discrepancia para advertir al usuario de una posible afectacion en la confiabilidad del proveedor.
...
- El sistema debe validar que el proveedor exista y este activo
- El sistema debe validar que el producto exista y este activo
- El sistema debe validar que la cantidad sea mayor a 0
- El sistema debe validar que la fecha de emisión sea igual o mayor a la fecha actual
- El sistema debe validar que la fecha de llegada sea igual o mayor a la fecha de emisión
- Las transacciones de ingreso son parte las entradas al sistema, pueden se manuales, via csv o via API.
